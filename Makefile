...
.PHONY: $(TARGET_INIT) $(TARGET_CLUSTER) $(TARGET_CLUSTER_OUTPUT) $(TARGET_CLUSTER_CLEAN) $(TARGET_PKI) $(TARGET_KUBECONFIG) $(TARGET_KUBECTL) $(TARGET_ETCD) $(TARGET_APISERVER) $(TARGET_CONTROLLER_MANAGER) $(TARGET_SCHEDULER) $(TARGET_KUBELET_AUTH) $(TARGET_MASTER) $(TARGET_WORKER_DEPS) $(TARGET_CNI) $(TARGET_CR) $(TARGET_KUBELET) $(TARGET_KUBE_PROXY) $(TARGET_DNS) $(TARGET_WORKER) $(TARGET_CLEAN) $(TARGET_ALL)
...
$(TARGET_CLUSTER):
	@cd $(IAC_PATH) && \
		$(TERRAFORM_ENV_VARS) $(TERRAFORM_BIN) apply -auto-approve
$(TARGET_OUTPUT):
	@cd $(IAC_PATH) && \
...
$(TARGET_CNI):
	...
	$(SCRIPTS_PATH)/worker/weavenet.sh
$(TARGET_CR):
...
$(TARGET_DNS):
	@$(SCRIPTS_PATH)/worker/dns.sh
$(TARGET_WORKER): $(TARGET_WORKER_DEPS) $(TARGET_CNI) $(TARGET_CR) $(TARGET_KUBELET) $(TARGET_KUBE_PROXY) $(TARGET_DNS)
$(TARGET_CLEAN):
	@rm -f $(DATA_PATH)/kubeconfig/* && \
	rm -f $(DATA_PATH)/pki/* && \
	cd $(IAC_PATH) && \
		$(TERRAFORM_ENV_VARS) $(TERRAFORM_BIN) destroy -auto-approve
$(TARGET_ALL): $(TARGET_INIT) $(TARGET_CLUSTER) $(TARGET_PKI) $(TARGET_KUBECONFIG) $(TARGET_MASTER) $(TARGET_WORKER) $(TARGET_OUTPUT)
